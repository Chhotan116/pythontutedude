...
# Menu / name input
# -------------------------
def button(surface, rect, text, inactive_col, active_col):
    mx, my = pygame.mouse.get_pos()
    r = pygame.Rect(rect)
...
def main_menu():
    ...
    # start bg music if not already playing (safe)
    if pygame.mixer.get_init() and find_first_existing(CANDIDATE_BG):
        try:
            if not pygame.mixer.music.get_busy():
                pygame.mixer.music.play(-1)
        except Exception as e:
            print("Audio: play bg music failed ->", e)
    ...
    while True:
        ...
        if show_instructions:
            inst = SMALL.render("Special Fruits: Gold(+5), Slow, Fast, Life(+1).", True, BLACK)
